...
func main() {
	...
	evalnews := make([]*bootstrapping.Evaluator, 128)
	ctsbefore := make([]*rlwe.Ciphertext, 128)
	for i := 0; i < 128;i ++{
		ctsbefore[i] = ciphertext1.CopyNew()
	}
	for i := 0; i < 128;i++ {
		evalnews[i] = eval.ShallowCopy()
	}
	cts := make([]*rlwe.Ciphertext, 128)

	Start := time.Now()
	ct, _ := evalnews[4].Bootstrap(ctsbefore[4])
	cts[4] = ct
	// for i := 0; i < 1; i++ {
	// 	wg.Add(1)
	// 	go func(i int) {
	// 		defer wg.Done()
	// 		ct, _ := evalnews[i].Bootstrap(ctsbefore[i])
	// 		cts[i] = ct
	// 	}(i)
	// }
	// wg.Wait()
	...
	printDebug(params, cts[4], valuesTest1, decryptor, encoder)
}
