...
func main() {
	...
	evalnews := make([]*bootstrapping.Evaluator, 8)
	ctsbefore := make([]*rlwe.Ciphertext, 8)
	for i := 0; i < 8;i ++{
	...
	cts := make([]*rlwe.Ciphertext, 8)
	// ct, _ := eval.Bootstrap(ctsbefore[9])
	...
	printDebug(params, cts[5], valuesTest1, decryptor, encoder)
}
